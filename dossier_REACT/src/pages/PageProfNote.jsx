...
const PageProfNote = () => {
  ...
  const recupererEvaluations = async () => {
    try {
      const url = `https://lsaintecroi.zzz.bordeaux-inp.fr/API/bdd.php?action=evaluations_discipline_annee&discipline=${prof.Discipline}&annee=${annee}`;
  ...
  const handleValider = async () => {
    try {
      ...
      const url = `https://lsaintecroi.zzz.bordeaux-inp.fr/API/bdd.php?action=eleves_annee_discipline&annee=${annee}&discipline=${prof.Discipline}`;
      const response = await fetch(url);
      if (!response.ok) {
        throw new Error(`Erreur HTTP ${response.status}`);
      }
  ...
  const AddGrade = async () => {
    if (contenuAjoutNote == "Ajouter une note") {
      ...
      const url = `https://lsaintecroi.zzz.bordeaux-inp.fr/API/bdd.php?action=ajout_note&annee=${annee}&discipline=${prof.Discipline}`;
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>
                  <button onClick={AddGrade}>{contenuAjoutNote}</button>
                </th>
                <th></th>
              </tr>
              <tr>
            ...
            <tbody>
              {eleves.map((eleve, index) => (
                <tr key={index}>
                  ...
                  <td>
                    <input type="number" disabled={!ajoutNote} />
                  </td>
